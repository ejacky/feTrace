...
type Config struct {
	...
	DeepseekConnectTimeout  time.Duration `json:"DEEPSEEK_CONNECT_TIMEOUT"`
	DeepseekReadTimeout     time.Duration `json:"DEEPSEEK_READ_TIMEOUT"`
...
var defaultConfig = Config{
	...
	FlushIntervalSec:       30,
	DeepseekAPIKey:         "",
	DeepseekConnectTimeout: 15 * time.Second,
	DeepseekReadTimeout:    40 * time.Second,
	GeocodeEnabled:         true,
	GeocodeMaxCalls:        3,
...
func loadFromEnv(cfg *Config) {
	...
	if connectTimeout := os.Getenv("DEEPSEEK_CONNECT_TIMEOUT"); connectTimeout != "" {
		if t, err := strconv.Atoi(connectTimeout); err == nil {
			cfg.DeepseekConnectTimeout = time.Duration(t) * time.Second
		}
	}

	if readTimeout := os.Getenv("DEEPSEEK_READ_TIMEOUT"); readTimeout != "" {
		if t, err := strconv.Atoi(readTimeout); err == nil {
			cfg.DeepseekReadTimeout = time.Duration(t) * time.Second
		}
	}
