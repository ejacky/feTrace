...
func (c *CacheService) LoadFromDisk() error {
	...
	if err := decoder.Decode(&c.data); err != nil {
		// 如果文件为空或格式错误，视为无数据，初始化为空
		if err.Error() == "EOF" || err.Error() == "unexpected end of JSON input" {
			log.Printf("People data file %s is empty or invalid JSON, starting with empty cache", c.savePath)
			c.data = models.PeopleData{Persons: []models.Person{}}
			return nil
		}
