...
def _get_timeouts():
    # 从 config 获取超时，提供默认值
    try:
        connect = int(os.environ.get('DEEPSEEK_CONNECT_TIMEOUT', 15))
        read = int(os.environ.get('DEEPSEEK_READ_TIMEOUT', 40))
        return (connect, read)
    except Exception:
        return (5, 15)
...
def _get_retry_params():
    try:
        total = int(os.environ.get('DEEPSEEK_RETRY_TOTAL', 2))
        connect = int(os.environ.get('DEEPSEEK_RETRY_CONNECT', total))
        read = int(os.environ.get('DEEPSEEK_RETRY_READ', total))
        status = int(os.environ.get('DEEPSEEK_RETRY_STATUS', total))
        backoff = float(os.environ.get('DEEPSEEK_BACKOFF_FACTOR', 0.6))
        return {
            'total': total,
            'connect': connect,
            'read': read,
...
def _augment_events(events: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    # 填充经纬度（保守限流）
    max_calls = int(os.environ.get("GEOCODE_MAX_CALLS", 3))
    calls = 0
    for e in events:
        lat_empty = str(e.get("lat", "")).strip() == ""
        lon_empty = str(e.get("lon", "")).strip() == ""
        if (lat_empty or lon_empty) and calls < max_calls and bool(os.environ.get("GEOCODE_ENABLED", True)):
